...
@section('konten-admin')
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800 fw-bold"><strong>Tambah Penerbit</strong></h1>
    </div>


    <div class="card mb-4 bg-warning-subtle border-start border-black border-3">
        <div class="card-body">
            <form action="{{ route('simpan.penerbit') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="row mb-3 fw-bold">
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control" id="InputNamaPenerbit" name="name"
                            placeholder="Nama Penerbit" value="{{ old('name') }}">
                        @error('name')
                            <span class="text-danger">{{ $message }}</span>
                        @enderror
                    </div>
                    <div class="col-md-6">
                        <label for="inputFoto" class="form-label">Logo Penerbit</label><br>
                ...
                <div class="row mb-3 fw-bold">
                    <div class="col-md-6 mb-3">
                        ...
                        <textarea class="form-control" name="alamat" id="inputAlamat" rows="3">{{ old('deskripsi') }}</textarea>
                        @error('alamat')
                            <span class="text-danger">{{ $message }}</span>
                        @enderror
                    </div>
